...
async function openExternalTerminalWithCommand(cmd, backendUrl) {
  ...
  try {
    ...
    if (process.platform === "win32") {
      const winCmd = `cmd /c start powershell -NoExit -Command "cd '${mobileDir}'; $env:EXPO_PUBLIC_API_BASE_URL='${backendUrl}'; ${cmd.replace(/"/g, '"')}"`;
      await run(winCmd);
      return true;
    }
    const terminals = [
...
async function main() {
  ...
  const backendUrlFromEnv = readExpoUrlFromBackendEnv();
  if (!backendUrlFromEnv) {
    console.error("EXPO_PUBLIC_API_BASE_URL not found in backend/.env. Please set it to the Railway URL.");
    process.exit(1);
  }
  const backendUrl = backendUrlFromEnv;
  console.log("Using backend URL from backend/.env:", backendUrl);
