...
export function queryHomePostsPageSync(args: {
	...
	const items: Post[] = (rows as any[]).map((r: any) => postFromRow(r));
...
export function queryProfileFeedPageSync(args: {
	...
	if (args.tab === "posts") {
		...
		const rows = db.getAllSync<any>(sql, params);
		const posts: Post[] = (rows as any[]).map((r: any) => postFromRow(r));
		hydratePostImagesSync(db, posts);
		const items: ProfileFeedItem[] = (rows as any[]).map((r: any, i: number) => ({
			kind: r.kind,
			post: posts[i],
		...
		const nextCursor = rows.length === limit ? makeFeedCursor(items[items.length - 1]) : null;
		return { items, nextCursor };
	}
	...
	if (args.tab === "media") {
		...
		const posts: Post[] = (rows as any[]).map((r: any) => postFromRow(r));
		hydratePostImagesSync(db, posts);
		const items: ProfileFeedItem[] = (rows as any[]).map((r: any, i: number) => ({ kind: "post", post: posts[i], activityAt: String(r.activityAt) }));
