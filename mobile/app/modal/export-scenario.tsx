...
export default function ExportScenarioModal() {
  ...
  const subtitle = useMemo(() => {
    const parts: string[] = [];
    if (includeProfiles) parts.push("profiles");
    if (includePosts) parts.push("posts");
    if (includeSheets) parts.push("sheets");

    if (parts.length === 0) return "Exports scenario settings only.";
    return `Exports scenario settings and ${parts.join(", ")}.`;
  }, [includeProfiles, includePosts, includeSheets]);
  ...
  }, [app, busy, scenarioId, includeProfiles, includePosts, includeSheets, onClose]);
